...
def LoadData(filename, max_z, input_vert_vars, output_vert_vars, all_ys=True, ind_y=None, n_trn_exs=None,
    ...
    for var in varis:
        ...
        if (v[var].shape[0] > 1 and len(v[var].shape) == 3 and v[var].shape[0]!=360 and v[var].shape[0]!=45): #Yani: check that the variable has all levels - otherwize it assumes that it has no z dimensions.
            if all_ys:
                if no_cos:
        ...
        elif ((v[var].shape[0] == 360 or v[var].shape[0] == 45) and len(v[var].shape) == 3):  # only 1 z level but with x dim input
            if all_ys:
                if no_cos:
