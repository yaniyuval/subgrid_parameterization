...
def build_training_dataset(expt, start_time, end_time, interval, n_x_samp=5, train_size=0.9, do_shuffle=True,
    ...
    filename_wildcard = input_dir + expt + 'km12x576_576x1440x48_ctl_288_' + str(start_time).zfill(10) + '_000*_diff_coarse_space_corrected_tkz' + str(flag_dict['resolution']) + '.nc4'  # New version of data
...
##1)a) If I decide that tabs is the correct field I should use, I should think if I also want to use Q,and QP before the time step took place. I don't think that this is what I currently do, but I should check what the Fortran uses, and see what I have used in the training procedure...
        ...
        if flag_dict['ver_adv_correct']:
            for k in range(n_z - 1):
                ...
                if flag_dict['do_qp_as_var']:
                    zqpout[k, :, :] = -(qpflux_z[k + 1, :, :] - qpflux_z[k, :, :]) / rho_dz[k]
            ...
            if flag_dict['do_qp_as_var']:
                zqpout[n_z - 1, :, :] = -(0.0 - qpflux_z[n_z - 1, :, :]) / rho_dz[n_z - 1]


        if flag_dict['do_dqp']:
            for k in range(n_z):
                if flag_dict['do_qp_as_var']:
                #     zTout[k, :, :] = zTout[k, :, :] #No effect of dqp on tfull!
                    zqpout[k, :, :] = zqpout[k, :, :] + dqp[k, :, :]
                else:
        ...
        if flag_dict['do_qp_as_var']: # I think that I need to include the fall tendency only if I use qp (it affects tfull and qp I think)/
            zTout = zTout + t_fall #I think it affects only the tfull and not T
            zqout = zqout  # + dqp_fall # This is a tendency that affects qp and not q....
            zqpout = zqpout + dqp_fall # This is a tendency that affects qp and not q....JY


        # Yani added the advection by vertical diffusion - Can consider later to do the correction for the horizontal advection. In the Fortran it seems that the bottom fluxes of the diffusion are calculated differently


        if flag_dict['do_z_diffusion_correction']:
            for k in range(n_z - 1):
                zqout[k, :, :] = zqout[k, :, :] - (qtflux_diff_z[k + 1, :, :] - qtflux_diff_z[k, :, :]) / rho_dz[k]
                zTout[k, :, :] = zTout[k, :, :] - (tflux_diff_z[k + 1, :, :]- tflux_diff_z[k, :, :])/ rho_dz[k]
                zqpout[k, :, :] = zqpout[k, :, :] - (qpflux_diff_z[k + 1, :, :] - qpflux_diff_z[k, :, :]) / rho_dz[k]

            zTout[n_z - 1, :, :] = zTout[n_z - 1, :, :] - (0.0 - tflux_diff_z[n_z - 1, :, :] + 0.0) / rho_dz[n_z - 1]
            zqout[n_z - 1, :, :] = zqout[n_z - 1, :, :] - (0.0 - qtflux_diff_z[n_z - 1, :, :]) / rho_dz[n_z - 1]
            zqpout[n_z - 1, :, :] = zqpout[n_z - 1, :, :] - (0.0 - qpflux_diff_z[n_z - 1, :, :]) / rho_dz[n_z - 1]

        # Horizontal advection -# Didn't do yet horizontal advection...
        rhow_num_y = np.tile(rho, [n_x, 1]).T
        for j in range(n_y - 1):
            zTout[:, j, :] = zTout[:, j, :] - (tflux_y[:, j + 1, :] - tflux_y[:, j, :]) / (dy * rhow_num_y)
            zqout[:, j, :] = zqout[:, j, :] - (qtflux_y[:, j + 1, :] - qtflux_y[:, j, :]) / (dy * rhow_num_y)

        zTout[:, n_y - 1, :] = zTout[:, n_y - 1, :] - (0.0 - tflux_y[:, n_y - 1, :]) / (dy * rhow_num_y)
        zqout[:, n_y - 1, :] = zqout[:, n_y - 1, :] - (0.0 - qtflux_y[:, n_y - 1, :]) / (dy * rhow_num_y)

        rhow_num_x = np.tile(rho, [n_y, 1]).T
        for i in range(n_x - 1):
            ib = i - 1
            zTout[:, :, i] = zTout[:, :, i] - (tflux_x[:, :, i + 1] - tflux_x[:, :, i]) / (dx * rhow_num_x)
            zqout[:, :, i] = zqout[:, :, i] - (qtflux_x[:, :, i + 1] - qtflux_x[:, :, i]) / (dx * rhow_num_x)

        zTout[:, :, n_x - 1] = zTout[:, :, n_x - 1] - (tflux_x[:, :, 0] - tflux_x[:, :, n_x - 1]) / (dx * rhow_num_x)
        zqout[:, :, n_x - 1] = zqout[:, :, n_x - 1] - (qtflux_x[:, :, 0] - qtflux_x[:, :, n_x - 1]) / (dx * rhow_num_x)

        # Horizontal diffusion
        for j in range(1, n_y):
            jb = j - 1
            zTout[:, j, :] = zTout[:, j, :] - (tflux_diff_coarse_y[:, j, :] - tflux_diff_coarse_y[:, jb, :]) / (dy)
            zqout[:, j, :] = zqout[:, j, :] - (qtflux_diff_coarse_y[:, j, :] - qtflux_diff_coarse_y[:, jb, :]) / (dy)

        zTout[:, 0, :] = zTout[:, 0, :] - (0.0 - tflux_diff_coarse_y[:, 0, :]) / (dy)
        zqout[:, 0, :] = zqout[:, 0, :] - (0.0 - qtflux_diff_coarse_y[:, 0, :]) / (dy)
        ...
        for i in range(n_x - 1):
            if i == 0:
                ...
                ib = i - 1
            zTout[:, :, i] = zTout[:, :, i] - (tflux_diff_coarse_x[:, :, i] - tflux_diff_coarse_x[:, :, ib]) / (dx)
            zqout[:, :, i] = zqout[:, :, i] - (qtflux_diff_coarse_x[:, :, i] - qtflux_diff_coarse_x[:, :, ib]) / (dx)

        # Add sedimentation
        zTout[0:flag_dict['sed_level'], :, :] = zTout[0:flag_dict['sed_level'], :, :] + cloud_lat_heat[0:flag_dict['sed_level'], :, :]
        zqout[0:flag_dict['sed_level'], :, :] = zqout[0:flag_dict['sed_level'], :, :] + cloud_qt_tend[0:flag_dict['sed_level'], :, :]

        # additional term from variation of latent heat of condensation
        # only account for variation in z here

        if not flag_dict['do_qp_as_var']:
            ...
            #CORRECTED RECENTLY  - THIS SHOULD HAVE BEEN COMMENTED!
        ...
        if flag_dict['sed_level'] < n_z: #Otherwise there is no need to assign 0 values...
            cloud_qt_tend[flag_dict['sed_level']:n_z, :, :] = 0.0  # I need it for later when I do startospheric correction
            print(p[flag_dict['strat_corr_level']])

        for k in range(flag_dict['strat_corr_level'],
                       n_z):  # Should I include the diffusive fluxes in the stratosphere ?
            if flag_dict[
                'do_t_strat_correction']:  # Correcting also T since when changing zq, it also changes T via latent heating - maybe the source of the waves?
    ...
    if flag_dict['do_qp_as_var']:
        qpin = np.reshape(qpin, (n_z_input, n_y, -1))

    Tout = np.reshape(Tout, (n_z, n_y, -1))
    qout = np.reshape(qout, (n_z, n_y, -1))
    if flag_dict['do_qp_as_var']:
        qpout = np.reshape(qpout, (n_z, n_y, -1))
...
def create_specific_data_string_desc(flag_dict):
    ...
    data_specific_description = str(flag_dict['do_dqp'])[0] + str(flag_dict['ver_adv_correct'])[0] + \
                                ...
                                str(flag_dict['qin_feature'])[0] + str(flag_dict['qin_z_grad_feature'])[0] + \
                                str(flag_dict['input_upper_lev']) + \
                                ...
                                str(flag_dict['calc_tkz_z'])[0] + str(flag_dict['calc_tkz_z_correction'])[0] + \
                                str(flag_dict['resolution']) + \
